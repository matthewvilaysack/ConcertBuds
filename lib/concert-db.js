...
export async function createOrJoinChatRoom(concertId, userId, username) {
  ...
  try {
    ...
    if (userConcertError) throw userConcertError;

    // If user has already joined, just return the chat room
    if (userConcert?.join_chat) {
    ...
    // If user hasn't joined, proceed with joining logic
    ...
    if (!chatRoom) {
      // If room doesn't exist, create it with initial count of 1
      const { data: newRoom, error: createError } = await supabase
        .from("chat_rooms")
    ...
    // If room exists, increment the count
