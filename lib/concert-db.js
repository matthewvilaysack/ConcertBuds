...
export async function createOrJoinChatRoom(concertId, userId, username) {
  ...
  try {
    ...
    // If user has already joined, return the existing room
    if (userConcert?.join_chat) {
      const { data: existingRoom, error: existingError } = await supabase
        .from("chat_rooms")
        .select("*")
    ...
    // Get or create chat room in a single operation using upsert
    const { data: chatRoom, error: upsertError } = await supabase
      .from("chat_rooms")
      .upsert(
        { 
          concert_id: concertId,
    ...
    // If room already existed, increment num_users
    if (chatRoom.num_users > 1) {
      const { data: updatedRoom, error: incrementError } = await supabase
        .from("chat_rooms")
        .update({ num_users: chatRoom.num_users + 1 })
        .eq("concert_id", concertId)
        .select()
        .single();
